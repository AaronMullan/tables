...
const ProgressBar: React.FC<ProgressBarTypes> = ({ progressBarColor, progressBarNumerator, progressBarDenominator, progressBarAsStars }) => {
  // convert ratio to percentage for regular progress bar
  const progressPercentage = (progressBarNumerator / progressBarDenominator) * 100;
  // convert ratio to percinquage for progress bar as stars
  const progressPercinquage = (progressBarNumerator / progressBarDenominator) * 5;
  const label = progressBarAsStars ? `${progressPercinquage} / 5` : `${progressBarNumerator} / ${progressBarDenominator}`;
  ...
  return (
    <Flex sx={{ alignContent: 'center', flexDirection: 'column' }}>
      <Box>
        {progressBarAsStars ? (
          <Flex sx={{ justifyContent: 'center', alignItems: 'center' }}>
            ...
            {Array.from({ length: Math.floor(progressPercinquage) }, (_, index) => (
              <Star key={index} />
            ))}
          </Flex>
        ) : (
      ...
      <label sx={{ fontSize: '14px', lineHeight: 1, mt: '4px', color: 'hsla(240, 6%, 42%, 1)', textAlign: 'center' }}>{label}</label>
    </Flex>
  );
};
